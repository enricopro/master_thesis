...
class DQNAgent:
    def __init__(self, action_space_size, state_space_shape, learning_rate=0.0001, discount_factor=0.99, exploration_rate=1.0, exploration_decay=0.99, min_exploration_rate=0.01, loaded=False, model_path="./models/DQN"):
    ...
    def update_q_values(self, state, action, reward, next_state, dones, action_mask):
        # Convert and cast all inputs as per the working pattern
        ...
        dones = tf.cast(dones, dtype=tf.float32)  # Cast as float32 immediately
        ...
        action_mask = tf.convert_to_tensor(action_mask, dtype=tf.float32)

        # Convert dones to a binary indicator (0 for done, 1 for not done)
        dones = 1.0 - dones  # Assuming 1 for ongoing, 0 for done
        
        # Predict Q-values for the next state
        next_state_q_values = self.model(next_state, training=False)

        # Filter Q-values by action_mask
        ...
        with tf.GradientTape() as tape:
            ...
            # Predict current Q-values for the current state
        ...
        # Get gradients and apply them to the model's trainable variables
