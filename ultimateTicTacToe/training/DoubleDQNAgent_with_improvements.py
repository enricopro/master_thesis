...
def dense_residual_block(x, units=256):
    """Two-layer residual MLP block: x → Dense-ReLU → Dense + skip → ReLU."""
...
class DoubleDQNAgent:
    ...
    def build_model(self):
        inputs = layers.Input(shape=self.state_space_shape)

        # First dense layer to lift the 81-vector to a richer space
        x = layers.Dense(256, activation='relu')(inputs)

        # === six residual blocks ===
        for _ in range(6):
            x = dense_residual_block(x, units=256)

        # Optional batch-norm for a bit more stability
        x = layers.LayerNormalization()(x)

        # Final linear head -> Q-values (one per action)
